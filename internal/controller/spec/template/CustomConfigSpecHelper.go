...
func CreateCustomConfigSpecHelper(kubegresContext ctx.KubegresContext, resourcesStates states2.ResourcesStates) CustomConfigSpecHelper {
...
func (r *CustomConfigSpecHelper) ConfigureStatefulSet(statefulSet *v1.StatefulSet) (hasStatefulSetChanged bool, differenceDetails string) {
	...
	if r.updateVolumeMountNameIfChanged(configMap.ConfigLocations.PostgreConf, states2.ConfigMapDataKeyPostgresConf, statefulSet) {
		differenceDetails += r.createDescriptionMsg(configMap.ConfigLocations.PostgreConf, states2.ConfigMapDataKeyPostgresConf)
		hasStatefulSetChanged = true
	}

	if r.updateVolumeMountNameIfChanged(configMap.ConfigLocations.PrimaryInitScript, states2.ConfigMapDataKeyPrimaryInitScript, statefulSet) {
		differenceDetails += r.createDescriptionMsg(configMap.ConfigLocations.PrimaryInitScript, states2.ConfigMapDataKeyPrimaryInitScript)
		hasStatefulSetChanged = true
	}

	if r.updateVolumeMountNameIfChanged(configMap.ConfigLocations.PgHbaConf, states2.ConfigMapDataKeyPgHbaConf, statefulSet) {
		differenceDetails += r.createDescriptionMsg(configMap.ConfigLocations.PgHbaConf, states2.ConfigMapDataKeyPgHbaConf)
		hasStatefulSetChanged = true
	}
